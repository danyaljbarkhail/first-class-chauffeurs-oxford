...
import { InsertUser, users, bookings, inquiries, Booking, Inquiry } from "../drizzle/schema";
 ...
 */
export async function createBooking(booking: typeof bookings.$inferInsert): Promise<Booking | null> {
  const db = await getDb();
  if (!db) {
  ...
  try {
    ...
    // Return success indicator - actual ID retrieval handled by database
    return { id: 1 } as any;
  } catch (error) {
 ...
 */
export async function createInquiry(inquiry: typeof inquiries.$inferInsert): Promise<Inquiry | null> {
  const db = await getDb();
  if (!db) {
